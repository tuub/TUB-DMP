<tr>
    ...
    <td>
        <strong>Project:</strong> {{ $plan->project->identifier }}<br/>
        ...
        @if( $plan->project->prefilled_at == $plan->project->updated_at )
            <strong>Auto Import Date:</strong> @date( $plan->project->prefilled_at ) at @time( $plan->project->prefilled_at )<br/>
        @else
            @if( $plan->project->prefilled_at )
                <strong>Auto Import Date:</strong> @date( $plan->project->prefilled_at ) at @time( $plan->project->prefilled_at )<br/>
            @endif
            ...
            @endif
        @endif
        <strong>Owner:</strong> {{ $plan->project->user->real_name }}
    </td>
    <td>
        <div>
            @if( !$plan->is_final )
            ...
            @if( $plan->is_final )
                ...
                @endif
            @else
                <a href="{{ URL::route('toggle_plan', [$plan->project->identifier, $plan->version]) }}" class="btn btn-default btn-xs" title="Finish"><i class="fa fa-lock"></i><span class="hidden-xs"> Finish DMP</span></a> and create version {{ $plan->version+1 }}
            @endif
        </div>
